...
final class PdoDb {	
	...
	public function init($key){
		...
		try{
			if($config['type'] == 'sqlite'){
				...
				if(!$config['dbname']){
					$config['dbname'] = PATH_PTAPP."/Data/data.db";
				}
				PtPath::mkdir(dirname($config['dbname']));
				$dsn = $config['type'].":".$config['dbname'];
				$this->conn = new PDO($dsn);
			}else{
				//PHP版本小于5.3.6，我需要在连接字符串中添加charset参数。
				$dsn = $config['type'].":host=".$config['host'].";charset=utf8;dbname=".$config['dbname'].";port=".$config['port'];
			...
			$pre = $e->getTrace();
			//throw new PtException($e->getMessage(),0,$pre[0]);
		}
	...
	public static function getObj($key = 'default')
	{				
	...
	public function run($sql,$args = array(),$return = FALSE,$returnType = 'one',$fetcheType = PDO::FETCH_ASSOC) {
		//$GLOBALS['run_dbquery_count'] += 1;
		//$GLOBALS['run_dbquery_sql'][] = array('sql'=>$sql,'args'=>$args);		
		//$this->showQuery($sql,$args);
		...
		try{
			...
			//print_pre($this->stm->debugDumpParams());
...
class MysqliDb {
	...
	function mysqli_query($sql){
		$query = mysqli_query($this->mysqli ,$query) or die($this->show_errors('Query Execution Error: '.mysqli_error($this->connection),'E'));
		return $query;
	}
	...
	public function getTables(){
		$query = mysqli_query($this->mysqli ,"SHOW TABLES");
